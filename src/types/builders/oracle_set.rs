...
use crate::types::{
    ...
    validation::validate_currency_code,
        ...
        if self.price_data_series.is_empty() {
            return Err(BuildError::Validation(
                crate::types::validation::ValidationError::InvalidAmount(
                    "PriceDataSeries must not be empty".into(),
                ),
            ));
        }
        for entry in &self.price_data_series {
            validate_currency_code(&entry.price_data.base_asset, false)?;
            validate_currency_code(&entry.price_data.quote_asset, false)?;
        }
        Ok(())
    }
