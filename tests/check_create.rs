...
async fn test_check_create_basic() {
    ...
    let sub_req =
        AccountTransactionsSubscription::validated(vec![sender_addr.clone()])
    ...
    client
        .request(SubmitRequestBuilder::new(&check_tx, &wallet).build().unwrap())
        .await
        .unwrap();
    ...
    let result = client
        .request(
            SubmitRequestBuilder::new(&cancel_tx, &wallet).build().unwrap(),
        )
        .await
        .unwrap()
        .result()
