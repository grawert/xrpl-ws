...
use crate::types::{
    validation::{
        validate_address, validate_domain, validate_email_hash,
        validate_message_key,
    },
    transactions::account::AccountSet,
...
impl AccountSetBuilder {
    ...
    pub fn with_domain(mut self, domain: impl AsRef<str>) -> Self {
        self.transaction_type.domain = Some(domain.as_ref().to_string());
        self
    }

    /// Sets the MD5 hash of the account's email address for Gravatar lookup.
    pub fn with_email_hash(mut self, hash: impl AsRef<str>) -> Self {
        self.transaction_type.email_hash = Some(hash.as_ref().to_string());
        self
    }

    /// Sets the hex-encoded public key for encrypted messaging.
    pub fn with_message_key(mut self, key: impl AsRef<str>) -> Self {
        self.transaction_type.message_key = Some(key.as_ref().to_string());
        self
    }
    ...
    pub fn with_nftoken_minter(mut self, minter: impl AsRef<str>) -> Self {
        self.transaction_type.nftoken_minter =
            Some(minter.as_ref().to_string());
        self
    }
}
...
impl TransactionTypeBuilder for AccountSet {
    ...
    fn validate(&self) -> Result<(), BuildError> {
        ...
        if let Some(message_key) = &self.message_key {
            validate_message_key(message_key)?;
        }
