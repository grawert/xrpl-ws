...
/// Clones a [`Memo`] from a reference.
impl From<&Memo> for Memo {
    fn from(m: &Memo) -> Self {
        m.clone()
    }
}
...
impl Memo {
    ...
    pub fn new(memo_data: impl AsRef<str>) -> Self {
        Self {
            memo_data: Some(memo_data.as_ref().to_string()),
            memo_format: None,
            memo_type: None,
        }
    }
    ...
    pub fn with_format(mut self, memo_format: impl AsRef<str>) -> Self {
        self.memo_format = Some(memo_format.as_ref().to_string());
        self
    }

    /// Sets the hex-encoded type/category identifier.
    pub fn with_type(mut self, memo_type: impl AsRef<str>) -> Self {
        self.memo_type = Some(memo_type.as_ref().to_string());
        self
    }
}
...
/// Wraps a [`Signer`] into the wire-format [`SignerWrapper`].
...
/// Unwraps a [`Signer`] from the wire-format [`SignerWrapper`].
...
/// Clones a [`Signer`] from a reference.
...
impl Signer {
    ...
    pub fn new(
        ...
        txn_signature: impl AsRef<str>,
        signing_pub_key: impl AsRef<str>,
    ) -> Self {
        Self {
            account: account.as_ref().to_string(),
            txn_signature: txn_signature.as_ref().to_string(),
            signing_pub_key: signing_pub_key.as_ref().to_string(),
        }
    }
}
...
/// Wraps a [`SignerEntry`] into the wire-format [`SignerEntryWrapper`].
