    ...
    TransactionBuilder<T>
{
    ...
    pub fn with_account_txn_id(mut self, id: impl Into<String>) -> Self {
        self.account_txn_id = Some(id.into());
        self
    }
    ...
    /// for example `(1 + N_signers) × base_fee` for multi-signature transactions.
    ...
    /// such as multi-signature transactions where the fee must be `(1 + N) × base_fee`.
    ///
    /// [`fill`]: Self::fill
