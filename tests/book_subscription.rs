...
async fn test_book_subscription_xrp_usd() {
    ...
    let (_resp, mut handle) = client
        ...
        .await
        .expect("Book subscription failed");

    // Wait for a transaction affecting the order book, with timeout.
    // The books stream delivers transaction messages (kind == "transaction"),
    // not bookChanges messages.
...
async fn test_book_subscription_multiple_books() {
    ...
    let result = timeout(Duration::from_secs(10), async {
        while let Ok(msg) = handle.recv().await {
            if msg.kind == "transaction" {
                println!("Received book transaction for multiple books setup");
                return true;
            }
        }
        false
