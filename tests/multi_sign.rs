...
async fn test_multi_sign_payment() {
    let seed1: Seed = test_seed(1).parse().expect("Failed to parse seed 1");
    let (priv1, pub1) =
        seed1.derive_keypair().expect("Failed to derive keypair 1");
    let wallet1 = Wallet { public_key: pub1, private_key: priv1 };
    let account1 = wallet1.public_key.derive_address();

    let seed2: Seed = test_seed(2).parse().expect("Failed to parse seed 2");
    let (priv2, pub2) =
        seed2.derive_keypair().expect("Failed to derive keypair 2");
    let wallet2 = Wallet { public_key: pub2, private_key: priv2 };
    let account2 = wallet2.public_key.derive_address();

    let seed3: Seed = test_seed(3).parse().expect("Failed to parse seed 3");
    let (priv3, pub3) =
        seed3.derive_keypair().expect("Failed to derive keypair 3");
    ...
    client
        ...
        .await
        .expect("Failed to submit SignerListSet");
    ...
    let mut payment_tx = payment_builder
        .with_fee(multisign_fee)
        .build()
        .expect("Failed to build payment");

    let sig1 =
        payment_tx.sign_as(&wallet1).expect("Failed to sign as wallet 1");
    let sig3 =
        payment_tx.sign_as(&wallet3).expect("Failed to sign as wallet 3");
    ...
    client
        .request(
            &SubmitRequestBuilder::new(&remove_tx, &wallet2)
                .build()
                .expect("Failed to build submit request"),
        )
        .await
...
async fn test_multi_sign_payment_with_ticket() {
    ...
    client
        ...
        .await
        .expect("Failed to submit TicketCreate");
