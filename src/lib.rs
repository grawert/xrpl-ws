...
//! ## Subscriptions
//!
//! Use [`Client::subscribe`] to receive a stream of validated transactions for
//! a specific account. After each transaction, [`util::available_balance`] returns
//! the spendable balance after reserves.
//!
//! ```no_run
...
//!
//! let result = client
//!     .request(SubmitRequestBuilder::new(&tx, &wallet).build()?)
//!     .await?;
//!
...
use socket::{request_pool, subscribe, SocketRequest};
...
#[derive(Clone)]
pub struct Client {
    /// The WebSocket endpoint URL (e.g. `"wss://xrplcluster.com"`).
    pub url: String,
    config: ClientConfig,
...
impl Client {
    ...
    pub fn with_config(url: impl Into<String>, config: ClientConfig) -> Self {
        let url = url.into();
        let connection = request_pool(url.clone(), config.clone());
    ...
    pub async fn request<T: XrplRequest>(
        ...
        self.connection
            .send(SocketRequest { payload, responder })
            .await
    ...
    /// Returns the subscription response and a subscription handle for receiving messages.
    pub async fn subscribe<T>(
        &self,
        sub: T,
    ) -> Result<(T::Response, SubscriptionHandle<T::Message>), XrplError>
    where
            ...
            + 'static,
    {
        ...
        let (responder, rx) = oneshot::channel();
        let req = SocketRequest { payload, responder };

        connection.send(req).await.map_err(|_| XrplError::Disconnected)?;
        ...
        // Create handle with dedicated subscription connection
        let handle = SubscriptionHandle::new(connection, typed_receiver);
