...
async fn test_payment_channel_create_basic() {
    ...
    let sub_req = AccountTransactionsSubscription::validated(vec![
    ...
    client
        .request(
            SubmitRequestBuilder::new(&channel_tx, &wallet).build().unwrap(),
        )
        .await
        .unwrap();
    ...
    let result = client
        .request(SubmitRequestBuilder::new(&claim_tx, &wallet).build().unwrap())
        .await
        .unwrap()
        .result()
        .unwrap();
