...
pub trait XrplSubscription: XrplRequest {
    ...
    /// Wire `"type"` tag carried by this subscription's push messages
    /// (e.g. `"ledgerClosed"`, `"transaction"`, `"bookChanges"`). Every working
    /// subscription's push messages carry a concrete tag, so this has no default.
