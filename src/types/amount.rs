...
/// Clones an [`Amount`] from a reference.
...
fn xrp_str_to_drops(xrp_str: &str) -> Result<u64, ValidationError> {
    ...
    let frac_drops: u64 = format!("{frac_str:0<6}").parse().expect(
        "frac_str is 0-6 ASCII digits, so the 6-char padded value fits in u64",
    );
...
impl Amount {
    ...
    {
        let value = value.as_ref().to_string();
        let currency = currency.as_ref().to_string();
    ...
    {
        let value = value.as_ref().to_string();
        let mpt_issuance_id = mpt_issuance_id.as_ref().to_string();
