...
// Order book activity is market-dependent, so unlike `ledger`/`book_changes`
// we can't reliably prove "no more pushes" - a quiet book proves nothing.
// These just confirm the unsubscribe/drop wire round-trip itself succeeds.

#[serial]
#[tokio::test]
async fn test_book_subscription_unsubscribe_succeeds() {
    let client = Client::new(server_url());
...
#[serial]
#[tokio::test]
async fn test_book_subscription_drop_does_not_error() {
    let client = Client::new(server_url());
    ...
    // No assertion beyond "this doesn't panic" - the drop-time unsubscribe
    // is fire-and-forget, so there is nothing to await here.
