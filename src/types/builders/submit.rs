...
impl<'a, W: SigningContext> SubmitRequestBuilder<'a, W> {
    ...
    pub fn build(self) -> Result<SubmitRequest, W::Error> {
        let mut req = SubmitRequest::new(self.tx.sign_with(self.wallet)?);
        if let Some(v) = self.fail_hard {
            req = req.with_fail_hard(v);
        }
        Ok(req)
    }
}
...
impl<'a> SubmitMultisignedRequestBuilder<'a> {
    ...
    pub fn build(self) -> Result<SubmitMultisignedRequest, serde_json::Error> {
        let tx_json = serde_json::to_value(self.tx)?;
        let mut req = SubmitMultisignedRequest::new(tx_json);
        if let Some(v) = self.fail_hard {
            req = req.with_fail_hard(v);
        }
        Ok(req)
